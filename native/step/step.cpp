// MeshThumbs STEP tessellator. MIT; Open CASCADE remains LGPL-2.1 with its exception.
...
EXPORT int meshthumbs_step_load(const std::uint16_t* path,
  ...
  try {
    if (!path || !sink || !triangleLimit) throw std::runtime_error("invalid STEP render request");
    ...
    if (!input) throw std::runtime_error("cannot open STEP file");
    input.seekg(0, std::ios::end);
    auto bytes = input.tellg();
    if (bytes <= 0 || bytes > 300LL * 1024 * 1024) throw std::runtime_error("STEP file size is invalid");
    input.seekg(0);
    ...
    if (reader.NbRootsForTransfer() <= 0 || reader.NbRootsForTransfer() > 100000)
      throw std::runtime_error("STEP root count is outside the supported limit");
    if (!reader.TransferRoots()) throw std::runtime_error("STEP contains no transferable geometry");
    const TopoDS_Shape shape = reader.OneShape();
    if (shape.IsNull()) throw std::runtime_error("STEP contains no shapes");

    Bnd_Box bounds;
    BRepBndLib::Add(shape, bounds, false);
    if (bounds.IsVoid() || bounds.IsOpen()) throw std::runtime_error("STEP has invalid bounds");
    double xmin, ymin, zmin, xmax, ymax, zmax;
    bounds.Get(xmin, ymin, zmin, xmax, ymax, zmax);
    const double cx = xmin / 2 + xmax / 2, cy = ymin / 2 + ymax / 2, cz = zmin / 2 + zmax / 2;
    ...
    // Meshing stays single-threaded to avoid unbounded work across Explorer requests.
    BRepMesh_IncrementalMesh mesh(shape, std::max(span * 0.001, 1e-7), false, 0.35, false);
    if (!mesh.IsDone()) throw std::runtime_error("STEP tessellation failed");

    std::uint64_t total = 0, faceCount = 0;
    for (TopExp_Explorer it(shape, TopAbs_FACE); it.More(); it.Next()) {
      if (++faceCount > 100000) throw std::runtime_error("STEP face count exceeds 100000");
      TopLoc_Location location;
      auto triangulation = BRep_Tool::Triangulation(TopoDS::Face(it.Current()), location);
      if (triangulation.IsNull() || triangulation->NbTriangles() <= 0)
        throw std::runtime_error("STEP has a face that could not be tessellated");
      total += static_cast<std::uint64_t>(triangulation->NbTriangles());
      if (total > triangleLimit) throw std::runtime_error("STEP exceeds the triangle limit");
    }
    if (!total) throw std::runtime_error("STEP contains no renderable faces");
    ...
    for (TopExp_Explorer it(shape, TopAbs_FACE); it.More(); it.Next()) {
      ...
      for (int i = 1; i <= triangulation->NbTriangles(); ++i) {
        ...
        if (reversed != transform.IsNegative()) std::swap(indices[1], indices[2]);
        double vertices[18];
        for (int v = 0; v < 3; ++v) {
          auto point = triangulation->Node(indices[v]).Transformed(transform);
          auto normal = triangulation->Normal(indices[v]).Transformed(transform);
          ...
          double values[] = {(point.X() - cx) / span, (point.Z() - cz) / span,
                             -(point.Y() - cy) / span, normal.X(), normal.Z(), -normal.Y()};
          for (int j = 0; j < 6; ++j) {
            if (!std::isfinite(values[j])) throw std::runtime_error("STEP has non-finite mesh data");
            vertices[v * 6 + j] = values[j];
          }
        }
        if (!sink(context, vertices)) throw std::runtime_error("STEP triangle transfer cancelled");
      }
    }
    return 0;
    ...
    if (error && errorCapacity) {
      ...
      std::strncpy(error, message ? message : "Open CASCADE failure", errorCapacity - 1);
